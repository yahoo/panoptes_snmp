...
/*
 * This structure is attached to the yahoo_panoptes_snmp.Session
...
PyMODINIT_FUNC initinterface(void)
{
    ...
    easysnmp_import = PyImport_ImportModule("yahoo_panoptes_snmp");
    if (easysnmp_import == NULL)
    {
        const char *err_msg = "failed to import 'yahoo_panoptes_snmp'";
        PyErr_SetString(PyExc_ImportError, err_msg);
        goto done;
    }

    easysnmp_exceptions_import = PyImport_ImportModule("yahoo_panoptes_snmp.exceptions");
    if (easysnmp_exceptions_import == NULL)
    {
        const char *err_msg = "failed to import 'yahoo_panoptes_snmp.exceptions'";
        PyErr_SetString(PyExc_ImportError, err_msg);
        goto done;
    }

    easysnmp_compat_import = PyImport_ImportModule("yahoo_panoptes_snmp.compat");
    if (easysnmp_compat_import == NULL)
    {
        const char *err_msg = "failed to import 'yahoo_panoptes_snmp.compat'";
        PyErr_SetString(PyExc_ImportError, err_msg);
        goto done;
    }
